...
class PaymentRentRepository implements IPaymentRentRepository {
  ...
  async create({ idClient, idCompany, IdNegotiation, value, parcel, linkComprovant, status, deadline, }: IPaymentRentDTO) {
    const payment: PaymentRent = new PaymentRent();
    const _id = uuidv4()
    Object.assign(payment, {
      _id, id: _id, idClient, idCompany, IdNegotiation, value, parcel, linkComprovant, status, deadline, created_at: new Date(),
    });
  ...
  async update({idClient, idCompany, IdNegotiation, value, parcel, linkComprovant, status, deadline,}): Promise<void> {}
}
