...
class MyPlainRepository implements IMyPlainRepository {
  ...
  async create({ idPlain, idCompany, status, value, maturity, namePlain, users, emphasis, }: IMyPlainDTO) {
    const payment: MyPlain = new MyPlain();
    const _id = uuidv4()
    Object.assign(payment, {
      _id, id: _id, idPlain, idCompany, status, value, maturity, namePlain, users, emphasis, created_at: new Date(),
    });
  ...
  async update({idPlain, idCompany, status, value, maturity, namePlain, users, emphasis,}): Promise<void> {}
}
